...
def generator_loss(logits_fake):
    ...
    loss = bce_loss(logits_fake, torch.ones(logits_fake.shape[0]))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    mse = nn.MSELoss()
    loss = mse(scores_real, torch.ones_like(scores_real)) + mse(scores_fake, torch.zeros_like(scores_fake))
    loss *= 0.5
...
def ls_generator_loss(scores_fake):
    ...
    mse = nn.MSELoss()
    loss = mse(scores_fake, torch.ones_like(scores_fake)) * 0.5
