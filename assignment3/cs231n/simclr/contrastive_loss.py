...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        s = sim(z_k, z_k_N) / tau
        sum_exp_s, sum_exp_s_inv = torch.tensor(0, dtype=torch.float), torch.tensor(0, dtype=torch.float)
        for i in range(2*N):
            sum_exp_s += torch.exp(sim(z_k, out[i]) / tau) if i != k else 0
            sum_exp_s_inv += torch.exp(sim(z_k_N, out[i]) / tau) if i != k+N else 0
        l1 = -s + torch.log(sum_exp_s)
        l2 = -s + torch.log(sum_exp_s_inv)
        total_loss += l1 + l2
...
def compute_sim_matrix(out):
    ...
    out_norm = torch.linalg.norm(out, dim=1)  # (2N, )
    sim_matrix = out @ out.T / out_norm / out_norm.view(-1, 1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = exponential.sum(dim=1).view(-1, 1)
    ...
    # sim_pos = sim_matrix[torch.arange(N), torch.arange(N) + N].view(-1, 1)  # Option 1
    sim_pos = sim_positive_pairs(out_left, out_right)  # Option 2
    ...
    numerator = sim_pos.repeat(2, 1) / tau
